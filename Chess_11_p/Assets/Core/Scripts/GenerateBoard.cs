...
public class GenerateBoard : MonoBehaviour
{
    ...
    public Material tileUblack;
    public Material tileUwhite;
    ...
    private void GenerateSingleTile(int x, int y)
    {
        ...
        Vector3 positionU = new Vector3(x, -0.01f, y);
        GameObject singleTile = Instantiate(tile, position, Quaternion.identity);
        GameObject singleTileU = Instantiate(tileU, positionU, Quaternion.identity);

        singleTile.transform.SetParent(transform);
        singleTileU.transform.SetParent(transform);

        singleTile.GetComponent<Renderer>().material = defaultMaterial;
        if(black)
        {
            singleTileU.GetComponent<Renderer>().material = tileUblack;
            black = false;
        }
        else
        {
            singleTileU.GetComponent<Renderer>().material = tileUwhite;
            black = true;
        }
